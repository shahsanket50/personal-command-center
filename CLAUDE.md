...
Integrates Claude API, Slack, Outlook/Microsoft 365 (office), Gmail (personal), Outlook Calendar (office), Google Calendar (personal) into one interface.
PostgreSQL is the data store — all writes go to Postgres. Notion is no longer used.

## Tech stack
- Frontend: React + Vite, Tailwind CSS, shadcn/ui
- Backend: Node.js + Express (server/ folder)
- AI: Claude API via Anthropic SDK (claude-sonnet-4-5 model)
- Data: PostgreSQL 16 via `pg` — server/services/db.js is the single data access layer
- Desktop: Electron 30, macOS tray icon (menubar app)

## How to run
- **Daily driver (Electron):** `npm start` — starts Postgres container, Express, Vite, and Electron in one command
- **Browser only:** `docker start cc-pg && npm run dev` — hot reload at http://localhost:5173
- **Docker Compose:** `docker-compose up --build` — fully self-contained, no Node install needed
- Migrations run automatically on server startup (CREATE TABLE IF NOT EXISTS — safe to re-run)

## Modules (build order)
0. Settings — accounts, OAuth, Postgres config, theme prefs
1. Claude CLI — dark theme, streaming, conversation logs → Postgres
2. Notes & Tasks — daily notes + task DB in Postgres, due dates, overdue reminders on open
3. Calendar — Microsoft Graph (office Outlook Calendar) + Google Calendar (personal) merged view
4. Morning Brief — generated on app open + manual trigger, written to Postgres
5. Slack digest — unread triage, action items → Postgres
6. Email triage — Outlook/Microsoft 365 (office) + Gmail (personal), action items → Postgres
