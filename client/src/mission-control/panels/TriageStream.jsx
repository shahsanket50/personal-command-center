      ...
      {chips.map((c) => {
        ...
        return (
          <button key={c.id} onClick={() => setActive(c.id)} onMouseDown={(e) => e.preventDefault()} style={{
            ...
            borderRadius: 10, cursor: 'pointer', fontSize: 12.5, fontWeight: 500, fontFamily: 'inherit', outline: 'none',
          }}>
...
export function TriageStream({ focused, cursor, filter, setFilter, compact, onLoaded }) {
  ...
  if (loading) return <div style={{ padding: '18px 12px', color: T.textGhost, fontSize: 13, fontFamily: 'ui-monospace, Menlo, monospace' }}>loading triage…</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
      ...
      <div className="mc-scroll" style={{ flex: 1, overflow: 'auto', fontFamily: 'ui-monospace, Menlo, monospace', fontSize: 13 }}>
        {sections.map((sec, si) => {
          ...
          return (
            <div key={sec.lane ?? 'flat'}>
              ...
              {isCollapsed
                ? <div onClick={() => toggleCollapse(sec.lane)} style={{ padding: '4px 11px', fontSize: 12, color: T.textGhost, cursor: 'pointer', borderBottom: `1px solid ${T.bg2}` }}>show {sec.items.length} {LANE_META[sec.lane]?.label}</div>
                : sec.items.map((it) => {
                    ...
                    return (
                      <div key={it.id} style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          {it.urgent && <AlertIcon />}
                          <span style={{ color: T.textGhost, marginLeft: 'auto', fontSize: 12 }}>{it.age}</span>
                        </div>
                        <div style={{ color: isCursor ? T.text : T.textDim, marginTop: 2, fontSize: 12.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingLeft: 17 }}>
